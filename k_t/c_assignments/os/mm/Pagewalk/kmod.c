...
long myIoctl(struct file *fp, unsigned int pid, unsigned long k)
{
	...
	pgd = pid1 -> mm -> pgd;
		...
		phy = (int)(((pgd + PGD) -> pgd) >> 12);
		pte = (pte_t *)kmap(&mem_map[phy]);

		printk("printf phy %lx\n",(pte + PT) -> pte);
		phy1 = (int)(((pte + PT) -> pte) >> 12);
		page = (unsigned int *)kmap(&mem_map[phy1]);

		page = (unsigned int *)((unsigned long int)page | off);
