...
static unsigned long myUnlocked_ioctl(struct file *, unsigned int, unsigned long);
...
static unsigned long myUnlocked_ioctl(struct file *fp, unsigned int pid, unsigned long a)
{
	struct task_struct *tp = NULL;
	
	tp = find_task_by_vpid(pid);
	printk("&task_struct = %x\n&mm_struct = %x\n", (int )tp, (int )tp->mm);
	return 0;	
}
