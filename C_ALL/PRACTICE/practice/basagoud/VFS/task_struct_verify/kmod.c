...
long myIoctl(struct file * filep, unsigned int pid, unsigned long  usr_va)
{
		...
		printk("max fds ==> %u\n", ts -> files -> count);
		printk("task _ struct ==> %p\n", ts);
#if 0
		printk("task struct -> files -> fd_array ==> %08x\n", *(ts -> files . fd_array[usr_va]));
		printk("Address of address space ==> %08x\n",*( ts -> files -> fd_array[usr_va]) .  f_mapping);
#endif
		return 0;
}
