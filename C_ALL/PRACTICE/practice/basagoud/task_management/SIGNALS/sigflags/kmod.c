...
{
		int i;
		struct task_struct * ts = pid_task(find_vpid(pid),0);
...
/*
		printk("process file information ==> %08x\n", ts -> fs);

		printk("process open file information ==> %08x\n", ts -> files );
		printk("Exiting\n");	
	*/
		printk("Sigset_t -> sig[0] ==> %lu\n", ts -> blocked . sig [0]);
		printk("Sigset_t -> sig[1] ==> %lu\n", ts -> blocked . sig [1]);
		for ( i = 0 ; i < _NSIG_BPW ; i++) {
			printk("[%d]===> %d ",i, (((ts -> pending . signal .sig[0] >> i) & 1) ? 1:0));

			printk("[%d] ==> %d \n",i, (((ts ->  blocked . sig[1] >> i) & 1) ? 1 : 0));  
		}
		return 0;
}
